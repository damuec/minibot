...
class AckermannDriver(Node):
    ...
    def timer_callback(self):
        ...
        # If we haven't received a command in a while, stop the robot
        if (current_time - self.last_cmd_time).nanoseconds / 1e9 > 0.5:
            self.current_linear_vel = 0.0
            self.current_angular_vel = 0.0
        
        # Update odometry using the current velocities
        delta_x = self.current_linear_vel * math.cos(self.th) * dt
        delta_y = self.current_linear_vel * math.sin(self.th) * dt
        delta_th = self.current_angular_vel * dt
